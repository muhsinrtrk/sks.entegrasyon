...
use App\Controller\Requests\Product\PostProductRequest;
use Closure;
use Exception;
use ReflectionClass;
    ...
    public function beforeAfter($command)
    {
        ...
        if (is_array($command)) {
            ...
            foreach ($command as $value) {
                $response = $this->beforeAfter($value);
                if ($response !== true) {
                    return $response;
                    break;
                }
            ...
            $response = $this->runMiddleware($command, $resolvedMiddleware, $params, $info);
    ...
    protected function resolveCallbackParameters(Reflector $reflection, array $uriParams): array
    {
        ...
        foreach ($reflection->getParameters() as $key => $param) {
            ...
            if ((!is_null($class) && $class->isInstance($this->request))) {
                 ...
                 */
            } elseif ((!is_null($class) && $class->isSubclassOf(get_class($this->request)))) {
                $requestClass = $class->newInstance($this->request);
                $parameters[] = $requestClass;
